...
# load and tidy data
df <- read.csv("RASPDataRepository.csv")
...
#gather data
df1<- df %>% select (ID:SEX, `GTScore`:Responsibility) %>% 
...
dfa2 <- dfa %>% select(ID:SEX, `GTScore`:Responsibility) %>% gather(`GTScore`:Responsibility, key=Measure, value=Score) %>% 
...
dfcluster2a$Category <-factor(dfcluster2a$Category, levels=c(personalitydimensions)) 
...
dfcluster3<-dfcluster2c %>% group_by(Category, cluster, sd, mean) %>% summarise(Score=mean(Score)) %>% mutate(ClusterAve = if_else(Score<=(mean-.5*sd), "Low", if_else(Score>=(mean+.5*sd), "High", "Average")))
...
ui <- 
  ...
  dashboardPage (
        dashboardHeader(title="R_Dashboard"),
        ...
        dashboardBody(
          ...
          tabItems(
               ...
               tabItem(tabName = "personality",
                       ...
                       fluidRow(
                             wellPanel(title="Most Common Personality Profiles", plotOutput("plot5"))
                         ),
...
df2 <- eventReactive(input$go, { dfb %>% 
    ...
    Group = if_else(TotalScore>=69, "Above", if_else (TotalScore >=31, "Normal","Below"))) %>%
...
output$plot <- renderPlotly({
    ...
    scale_color_manual(name="", values=c( "Above"="lightgreen", "Normal"="gray", "Below" = "coral2")) +
...
output$table <- renderDataTable({
     df2() %>% select (ID, TotalScore, Physical, Cognitive, cluster, personalitydimensions) %>% 
    ...
    datatable(class = "display compact", filter = 'top', options = list (
      ...
      rownames= FALSE,
      colnames=c("Total" = "TotalScore", "Complex"="Complexity", "Innov"="Innovation", "Toleran"="Tolerance", "Cooper"="Cooperativeness", "Social"="Sociability", "Confid"="Confidence", "Astute"="Astuteness", "Organiz"="Organization", "Tradit"="Traditional", "Respon"="Responsibility"))
})
...
output$plot4a <- renderPlot({
  standardplot(df1, "GTScore", Score) + 
          xlab("GTScore") +
          theme(text = element_text(size = 14))
  })
...
output$plot6<- renderPlot ({
     dfPersonality %>% ggplot() +
        geom_boxplot(aes(x=Category, y=Score), fill="khaki") +
        ylab("Jackson T Score") + 
          xlab("JPI-R Dimension") +
          theme(text = element_text(size = 14))+ 
        theme(axis.text.x = element_text(angle = 30)) + ggtitle("Boxplot: Total Dataset")
