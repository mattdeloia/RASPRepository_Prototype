...
p <- df5 %>% group_by(ID, Category) %>% summarise(CyberFitScore=sum(CyberFit, na.rm = TRUE)) %>%  spread (key= "Category", value="CyberFitScore") %>% mutate(Fit = if_else((Personality > .7 & Cognitive > .7), "Best Fit", if_else( (Personality>.7|Cognitive>.7), "Good Fit", "Poor Fit"))) %>% 
     ggplot() +
